// views/student/cbt.js
// Bagian 2: Komponen CBT
// FINAL: Custom Dark Alert (No Native Browser Alert), Auto-Save, Anti-Reload

export const CBT_COMPONENTS = `
/* --- CSS GLOBAL --- */
const GLOBAL_STYLES = \`
...
/* --- COMPONENT: CUSTOM ALERT (PENGGANTI ALERT BROWSER) --- */
const CustomAlert = ({ isOpen, title, message, type = 'info', onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-sm p-6 text-center shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <button 
                    onClick={onClose}
                    className={\`w-full py-3.5 rounded-xl font-bold text-white shadow-lg transition active:scale-95 \${btnClass}\`}
                >
                    MENGERTI
                </button>
            </div>
...
/* --- MODAL 1: PERINGATAN (TIMER 5 DETIK) --- */
...
/* --- MODAL 2: DISKUALIFIKASI (LAYAR MERAH) --- */
...
const QuizReview = ({ quizId, studentId, onBack }) => {
...
/* --- MAIN ENGINE: QUIZ RUNNER --- */
const QuizRunner = ({ quizId, studentId, duration, onFinish }) => {
    ...
    // Custom Alert State (Global di dalam QuizRunner)
    ...
    // --- 1. START QUIZ & LOAD SAVED ANSWERS ---
    useEffect(() => {
        const start = async () => {
            try {
