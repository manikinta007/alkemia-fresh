// views/student/studentTasks.js
// Halaman Tugas Siswa (Worksheet Mode) - Wrapped for Portal Injection
// Update V11: Fix Review Mode (Pass student_id to detail API)

export const STUDENT_TASK_COMPONENT = `
    const StudentTasks = ({ student, onBack }) => {
        ...
        const openTask = async (taskId, status) => {
            ...
            try {
                // [FIX UPDATE V11] Tambahkan student_id ke parameter URL agar server bisa load jawaban lama
                const res = await fetch(\`/api/student/task-detail?task_id=\${taskId}&student_id=\${student.id}\`, { headers: getHeaders() });
                if (res.ok) {
                    const data = await res.json();
                    
                    // Set Active Task termasuk data Discussion Global
                    setActiveTask({ 
                        id: taskId, 
                        ...data.task, 
            ...
            showConfirm(isDraft ? 'Simpan Draft' : 'Kumpul Tugas', confirmMsg, async () => {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        } else showAlert('Tersimpan', 'Jawaban berhasil disimpan sementara.', 'success');
                    } else {
                                    ...
                                    {q.type === 'pg' && (
                                        <div className="space-y-2">
                                            {q.options.map((opt, oIdx) => {
                                                ...
                                                // [LOGIC WARNA PG]
                                                // Phase 1 (Key Locked): Netral (Biru jika selected)
                                                // Phase 2 (Key Open): Hijau (Benar), Merah (Selected Salah)
                                        ...
                                        <div>
                                            ...
                                            {isReadOnly && (
                                                <div className="mt-2 text-right">
                                                    {/* Nilai per soal Essay (Butuh data dari API detail answer, tapi di props kita belum inject score per item. 
                                                        Untuk saat ini kita tampilkan Bobot saja karena score per item ada di DB answer) */}
                                                    {/* Note: Idealnya API task-detail mengembalikan existing_answers beserta SCORE per item. */}
