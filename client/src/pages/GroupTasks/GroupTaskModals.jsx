...
import { X, Search, Check, AlertTriangle, Users } from 'lucide-react';

// --- 1. MODAL BUAT TUGAS KELOMPOK BARU ---
export const CreateGroupTaskModal = ({ isOpen, onClose, form, setForm, groupSets, onCreate, saving }) => {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 animate-in zoom-in-95">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl font-bold">Buat Tugas Kelompok Baru</h3>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Contoh: Proyek Praktikum Stoikiometri"
                        />
                    </div>
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder="Petunjuk pengerjaan kelompok..."
                        />
                    </div>
                    <div>
                    ...
                    <div className="p-4 bg-purple-50 border border-purple-200 rounded-xl">
                        <label className="text-xs font-bold text-purple-700 mb-2 block flex items-center gap-1">
                            <Users size={14} /> PILIH SET KELOMPOK
                        </label>
                        {groupSets.length === 0 ? (
                            <div className="text-center py-4 text-sm text-zinc-500">
                                <p className="font-bold mb-1">Belum ada set kelompok untuk kelas ini.</p>
                                <p className="text-xs text-zinc-400">Buat set kelompok di halaman Random Picker terlebih dahulu.</p>
                            </div>
                        ) : (
                            <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                                {groupSets.map(gs => (
                                    <label
                                        key={gs.id}
                                        className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition ${form.groupSetId === gs.id
                                    ...
                                    >
                                        <input
                                            type="radio"
                                            name="groupSet"
                                            checked={form.groupSetId === gs.id}
                                            onChange={() => setForm({ ...form, groupSetId: gs.id })}
                                            className="accent-purple-600"
                                        />
                                        <div className="flex-1">
                                            <span className="text-sm font-bold text-zinc-900">{gs.name}</span>
                                            <span className="ml-2 text-[10px] text-zinc-400">{gs.group_count || '?'} kelompok</span>
                                        </div>
                                    </label>
                                ))}
                            </div>
                        )}
                        {!form.groupSetId && groupSets.length > 0 && (
                            <p className="text-[10px] text-red-500 mt-2 font-bold flex items-center gap-1">
                                <AlertTriangle size={12} /> Wajib pilih 1 set kelompok
                            </p>
                        )}
                    </div>
                </div>
                <div className="flex gap-3 mt-6">
                    ...
                    <button
                        onClick={onCreate}
                        disabled={saving || !form.groupSetId}
...
// --- 2. MODAL KONFIGURASI BOBOT (DI GRADING) ---
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto flex flex-col">
                ...
                <div className="mb-4 flex-1 overflow-y-auto min-h-0">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        {essayCount > 0 && (
                            <button onClick={onDistribute} className="text-xs bg-zinc-100 text-zinc-600 px-3 py-1.5 rounded-lg font-bold hover:bg-zinc-200 transition border border-zinc-200">
...
// --- 3. MODAL GLOBAL KEY & DISCUSSION (MASTER KEY) ---
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 animate-in zoom-in-95">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <p className="text-xs text-zinc-500">Atur transparansi jawaban untuk kelompok.</p>
                    </div>
