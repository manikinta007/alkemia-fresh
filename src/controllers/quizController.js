...
export async function handleQuizRequest(request, env) {
    ...
    try {
        ...
        if (pathname === "/api/quizzes" && method === "POST") {
            ...
            const { periodId, classId, title, description, showResults, checkAttendance, allowedStudents, isOfflineMode } = body;

            await env.DB.prepare(`
            INSERT INTO quizzes (period_id, class_id, title, description, duration, tolerance_minutes, show_limit, is_random, show_results, is_active, check_attendance, allowed_students, is_offline_mode)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(
                periodId,
                classId,
        ...
        if (pathname === "/api/quizzes" && method === "PUT") {
            const body = await request.json();
            const { id, title, description, duration, toleranceMinutes, showLimit, isRandom, showResults, checkAttendance, allowedStudents, isOfflineMode } = body;

            await env.DB.prepare(`
            UPDATE quizzes 
            SET title = ?, description = ?, duration = ?, tolerance_minutes = ?, show_limit = ?, is_random = ?, show_results = ?, check_attendance = ?, allowed_students = ?, is_offline_mode = ?
            WHERE id = ?
        `).bind(
                title,
                description,
